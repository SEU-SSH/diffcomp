...
taskset -c 0 $PREFUZZ/afl-fuzz -i "$CORPUS_DIR" -o "$OUTPUT_DIR" -M master -t 5000 -- "$HARNESS" @@ &

# 从实例绑定至其他 CPU 核心
echo "[*] Starting $SLAVES Slave instances..."
for i in $(seq 1 $SLAVES); do
    taskset -c $i $PREFUZZ/afl-fuzz -i "$CORPUS_DIR" -o "$OUTPUT_DIR" -S slave$i -t 5000 -- "$HARNESS" @@ > "slave$i.log" 2>&1 &
done
